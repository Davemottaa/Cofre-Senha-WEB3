...
async function connectWallet() {
    if (!window.ethereum) {
        showError("❌ MetaMask Não Encontrada", "Você precisa instalar a extensão MetaMask no navegador.\n\n1. Visite: metamask.io\n2. Clique em \"Download\"\n3. Escolha seu navegador\n4. Siga as instruções de instalação");
        return;
    }
    try {
        showProcessing("🔐 A Conectar à Carteira", "Por favor, confirme na janela da MetaMask que aparece.");
        const provider = new ethers.providers.Web3Provider(window.ethereum);
        
        // Validar rede antes de conectar
        ...
        // Mensagem com timestamp para evitar replay attacks
        const message = `Aceder ao Cofre Seguro\nTimestamp: ${Date.now()}`;
        showProcessing("✍️ A Assinar Mensagem", "Confirme a assinatura na MetaMask (isto é seguro).");
        ...
        showProcessing("📥 A Carregar Senhas", "Aguarde enquanto recuperamos os seus dados...");
        await downloadFromBlockchain();
    } catch (e) { 
        showError("❌ Erro ao Conectar", "Não conseguimos conectar à sua carteira.\n\nCertifique-se de que:\n✓ MetaMask está instalada\n✓ Você clicou em \"Confirmar\"\n✓ Está na rede Sepolia");
    }
}
